...
# Test 1: detect_platform function exists in platform_detection.sh component
test_detect_platform_exists() {
  local content
  content=$(cat "$COMPONENTS_DIR/core/platform_detection.sh")
  
  assert_contains "$content" "detect_platform" "Component should define detect_platform function"
}
