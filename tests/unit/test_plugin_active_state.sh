...
test_active_plugin_is_discovered() {
  ...
  run_command output exit_code "$SCRIPT_PATH" -p list 2>&1 || true
...
test_plugin_list_shows_active_status() {
  ...
  assert_contains "$output" "ACTIVE\|✓\|enabled" "Should display active status indicator"
...
test_plugin_list_shows_inactive_status() {
  ...
  assert_contains "$output" "INACTIVE\|✗\|disabled" "Should display inactive status indicator"
...
EOF
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --config "$TEST_CONFIG_FILE" -p list 2>&1 || true
...
test_cli_flag_activates_plugin() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --activate-plugin cli-activate -p list 2>&1 || true
...
test_cli_flag_deactivates_plugin() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --deactivate-plugin cli-deactivate -p list 2>&1 || true
...
test_multiple_activate_flags() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --activate-plugin multi-activate-1 --activate-plugin multi-activate-2 -p list 2>&1 || true
...
test_multiple_deactivate_flags() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --deactivate-plugin multi-deactivate-1 --deactivate-plugin multi-deactivate-2 -p list 2>&1 || true
...
EOF
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --config "$TEST_CONFIG_FILE" --activate-plugin cli-override -p list 2>&1 || true
...
test_cli_flag_overrides_descriptor() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --activate-plugin cli-descriptor-override -p list 2>&1 || true
...
test_rename_to_disabled_deactivates() {
  ...
  run_command output1 exit_code1 "$SCRIPT_PATH" -p list 2>&1 || true
  ...
  run_command output2 exit_code2 "$SCRIPT_PATH" -p list 2>&1 || true
...
test_inactive_plugins_not_executed() {
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" -f "$test_file" 2>&1 || true
...
EOF
  ...
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --config "$TEST_CONFIG_FILE" --deactivate-plugin precedence-test -p list 2>&1 || true
...
test_activate_nonexistent_plugin_message() {
  local output exit_code
  run_command output exit_code "$SCRIPT_PATH" --activate-plugin nonexistent-plugin-xyz -p list 2>&1 || true
