...
## Acceptance Criteria
- [ ] A new file `doc.doc.md/components/plugin_info.py` exists and implements at minimum two callable modes: `tree <plugins_dir>` (DFS graph traversal with cycle detection and ASCII rendering) and `table <json_input>` (column-aligned table formatting for plugin command/parameter lists)
- [ ] `bsdextrautils` (i.e. the `column` command) is no longer required at runtime; the `column -t` invocation is removed from `plugin_management.sh` in favour of a call to `plugin_info.py`
- [ ] The Bash `cmd_tree` function (moved to `plugin_management.sh` by FEATURE_0027) is refactored into a thin wrapper that invokes `python3 .../plugin_info.py tree`; all DFS, cycle-detection, and rendering logic lives exclusively in Python
- [ ] The `plugin_info.py` tree and table functions are covered by at least five Python unit tests (using `unittest` or `pytest`) that do not require a shell environment
- [ ] All existing automated tests in `tests/` (shell-level integration tests) continue to pass without modification after the rewrite, confirming backward-compatible CLI output (REQ_0038)
- [ ] `plugin_info.py` carries a header comment defining its public CLI interface (argument syntax, exit codes, stdout contract) consistent with REQ_0037
- [ ] A graceful error is produced (non-zero exit, human-readable message to stderr) when `plugin_info.py` is invoked with an invalid or missing plugins directory, or malformed JSON input
...
- Requirements: [REQ_038 Backward-Compatible CLI](../../../02_project_vision/02_requirements/03_accepted/REQ_0038_backward-compatible-cli.md)
