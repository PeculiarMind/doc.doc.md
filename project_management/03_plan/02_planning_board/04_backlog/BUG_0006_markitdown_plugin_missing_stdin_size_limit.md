...
- [ ] `main.sh` reads at most 1,048,576 bytes from stdin (consistent with `head -c 1048576`)
- [ ] Payloads exceeding 1 MB are truncated; the plugin either processes the truncated JSON or exits with a JSON parse error (same behavior as other plugins)
- [ ] Payloads within the 1 MB limit continue to work correctly (regression test)
